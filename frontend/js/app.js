...
window.handleToggle = async function(id, isCompleted) {
...
window.handleUpdate = async function(id) {
    ...
    try {
        const task = allTasks.find((t) => t.id === id);
        await updateTask(id, title, desc, task?.isCompleted || false);
        await loadTasks();
    } catch (error) {
...
window.hideEditMode = function(id) {
    const item = document.getElementById(`task-${id}`);
    const viewMode = item.querySelector(".task-view");
    const editMode = document.getElementById(`edit-${id}`);
